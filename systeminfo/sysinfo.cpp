//HARDWARE INFO LIBRARY FOR GENiEBEN's APPLICATIONS v1.2
...
#include <QTime>
#include <QDateTime>
#include "systeminfo/smbios.hpp"    //baseline of SystemInfo
#include <QDebug>

QString _dxdiagpath;

//STORE SMBIOS VALUES
QString smbios_bios_vendor;
QString smbios_bios_version;
QString smbios_bios_release_date;
...
bool _getSMBIOS() {
    ...
    BYTE  *b_ {nullptr};
    smbios::dmi_header* header {nullptr};
    smbios::RawSMBIOSData* smbios_data     {nullptr};
    ...
    if (!smbios_data_size) {
        qWarning() << "SMBIOS Out of Memory";
        return false;
    }

    if (smbios_data_size == 0) {
        qWarning() << "Error encounters 'GetSystemFirmwareTable': " << GetLastError();
        return false;
    }
    ...
    if (!smbios_data_size) {
        ...
        qInfo() << "SMBIOS buffer size is: " << smbios_buffersize;
        smbios_data =reinterpret_cast<smbios::RawSMBIOSData*> (smbios_data_buffer);
        if (smbios_data->Length != smbios_buffersize - 8)
        {
            qWarning() << "SMBIOS length error";
            return false;
        }
    b_ = smbios_data->SMBIOSTableData;
    for (DWORD index{}; index < smbios_data->Length; index++) {

        smbios::dmi_header *header = reinterpret_cast<smbios::dmi_header*> (b_);

        if (header->type == smbios::dmi_entry_type::DMI_ENTRY_BIOS && flag) {
            qInfo() << "SMBIOS: BIOS";
            smbios_bios_vendor = dmi_string(header, b_[0x4]);
            qInfo() << "\t\tBIOS Vendor:    " << smbios_bios_vendor;
            smbios_bios_version = dmi_string(header, b_[0x5]);
            qInfo() << "\t\tBIOS Version:   " << smbios_bios_version ;
            smbios_bios_release_date = dmi_string(header, b_[0x8]);
            qInfo() << "\t\tRelease date:   " << smbios_bios_release_date;

            flag = 0;
        }

        else if (header->type == smbios::dmi_entry_type::DMI_ENTRY_BASEBOARD) {
            qInfo() << "SMBIOS: Baseboard";
            qInfo() << "\t\tManufacturer:   " << dmi_string(header, b_[0x4]);
            qInfo() << "\t\tProduct Number: " << dmi_string(header, b_[0x5]);
            qInfo() << "\t\tVersion:        " << dmi_string(header, b_[0x6]);
            qInfo() << "\t\tSerial Number:  " << dmi_string(header, b_[0x7]);
            qInfo() << "\t\tUUID:           ";

            smbios::dmi_system_uuid(b_ + 0x8, smbios_data->SMBIOSMajorVersion * 0x100 + smbios_data->SMBIOSMinorVersion);
        }


        else if (header->type == smbios::dmi_entry_type::DMI_ENTRY_MEM_DEVICE) {
            qInfo() << "SMBIOS: Memory";
            qInfo() << "\t\tSize of RAM:    " << dmi_string(header, b_[0xC]);
            qInfo() << "\t\tManufacturer:   " << dmi_string(header, b_[0x17]);
            qInfo() << "\t\tSerial Number:  " << dmi_string(header, b_[0x18]);
        }
...
bool _rungpuz() {
    ...
    gpuz.start("gpuz.exe", args);gpuz.waitForFinished(-1);gpuz.close();
...
QString SYSINFO_CPU_NAME() {
    ...
    //Trim and rename
    result.replace("11th Gen","",Qt::CaseSensitive);
    result.replace("12th Gen","",Qt::CaseSensitive);
...
QString SYSINFO_MOBO_VENDOR() {
    ...
    result.replace("Micro-Star International Co., Ltd.","MSI",Qt::CaseInsensitive);
    result = result.trimmed();
    return result;
}
...
QString SYSINFO_BIOS_VENDOR() {
    QString result= smbios_bios_vendor;
...
QString SYSINFO_BIOS_VERSION() {
    QString result= smbios_bios_version;
...
QString SYSINFO_RAM_PARTNUMBER() {
    ...
    result.replace("4400 C19 Series","Viper Steel DDR4-4400",Qt::CaseInsensitive);
    //
    result.replace("|||","-",Qt::CaseInsensitive);
    result.replace("||","-",Qt::CaseInsensitive);
    result.replace("| ","|", Qt::CaseInsensitive);
...
QString SYSINFO_RAM_VENDOR() {
    ...
    result.replace("Patriot Memory (PDP Systems)","Patriot",Qt::CaseInsensitive);
...
QString SYSINFO_GFX_NAME() {
    ...
    if (GPU_VendorID.at(i)=="8086"){
        ...
        boo.replace("Intel(R) UHD","UHD",Qt::CaseInsensitive);
